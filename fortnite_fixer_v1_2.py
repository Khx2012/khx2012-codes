...
class FortniteFixer_v1_2:
    def __init__(self, root):
        ...
        self.root.title("Fortnite Crash Fixer v1.2")
    ...
    def build_ui(self):
        ...
        ttk.Button(btn, text="🔍 Analyze Crash Logs",
                   command=self.analyze_crash_logs).pack(fill=tk.X, pady=3, padx=10)
        ttk.Button(btn, text="🔄 Reset Config Files",
                   command=self.reset_config).pack(fill=tk.X, pady=3, padx=10)
        ttk.Button(btn, text="⚠️ Check RAM Pressure",
                   command=self.check_ram_pressure).pack(fill=tk.X, pady=3, padx=10)
        ...
        ttk.Label(self.root,
                  text="Version 1.2  |  Not affiliated with Epic Games",
